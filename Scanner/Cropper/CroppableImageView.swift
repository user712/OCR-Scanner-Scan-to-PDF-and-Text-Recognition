...
class CroppableImageView: UIView, CornerpointClientProtocol
{
    ...
    var  imageToCrop: UIImage?
    {
        didSet
        {
            ...
            setNeedsLayout()
            cropDelegate?.imageSizeSetted(size: viewForImage.bounds.size)
        }
    }
    ...
    var  cropDelegate: CroppableImageViewDelegateProtocol?
    ...
    var cropRect: CGRect?
    {
        set
        {
            ...
            {
                let newRect:CGRect =  realCropRect.intersection(viewForImage.bounds)
    ...
    override func layoutSubviews()
    {
        ...
        if imageToCrop != nil
        {
            ...
            let path = UIBezierPath(rect: viewForImage.bounds)
            UIColor.red.setFill()
            path.fill()
            
            imageToCrop?.draw(in: viewForImage.bounds)
    ...
    override func draw(_ rect: CGRect)
    {
        ...
        if let realCropRect = internalCropRect
        {
            ...
            cropRect = CGRect(x: self.frame.width/2 - width/2, y: self.frame.height/2 - height/2, width: width, height: height)
            internalCropRect = cropRect
            draw(internalCropRect!)
        }
    }
    
    func selectAllArea() {
        cropRect = CGRect(origin: CGPoint.zero, size: imageSize!)
        internalCropRect = cropRect
        draw(CGRect.zero)
    }
    ...
    func handleDragInView(_ thePanner: UIPanGestureRecognizer)
    {
        ...
        case UIGestureRecognizerState.changed:
            ...
            if draggingRect
            {
                ...
                if newX + internalCropRect!.size.width > viewForImage.bounds.size.width
                {
                    newX = viewForImage.bounds.size.width - internalCropRect!.size.width
                }
                var newY = max(startPoint!.y + thePanner.translation(in: self).y,0)
                if newY + internalCropRect!.size.height > viewForImage.bounds.size.height
                {
                    newY = viewForImage.bounds.size.height - internalCropRect!.size.height
                }
...
//        }
    ...
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        ...
        for targetView in cornerpoints {
            let pointForTargetView = targetView.convert(point, from: self)
