...
extension Camera: AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    func captureOutput(_ captureOutput: AVCaptureOutput!, didOutputSampleBuffer sampleBuffer: CMSampleBuffer!, from connection: AVCaptureConnection!) {
        if isTakePhoto {
            ...
            if let ciImage = RectangleDetector.main.sampleBufferToImage(sampleBuffer) {
                ...
                DispatchQueue.main.async { [unowned self] in
                    if let lastRectFeature =  self.borderDetectLastRectangleFeature {
                        self.shapeLayer.draw(inRect: self.superView.frame, ciImage: ciImage, biggestRectangleFeature: lastRectFeature)
                    }
