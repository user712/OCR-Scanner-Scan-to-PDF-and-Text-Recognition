...
protocol VideoPreviewLayerProvider: class {
    /**
...
extension VideoPreviewLayerProvider {
...
protocol MetadataOutputDelegate: class {
    /**
...
class CaptureManager: NSObject {
    ...
    weak var metadataOutputDelegate: MetadataOutputDelegate? {
        didSet {
     ...
     */
    func authorizationStatus(forInput input: CaptureSessionInput) -> AVAuthorizationStatus {
     ...
     */
    func setUp(sessionPreset: String,  previewLayerProvider: VideoPreviewLayerProvider?, inputs: [CaptureSessionInput], outputs: [CaptureSessionOutput],
        ...
        func setUpCaptureSession() throws {
            ...
            dataOutput.setSampleBufferDelegate(self, queue: framesQueue)
            ///
        ...
        if let layerProvider = previewLayerProvider {
            ...
            layerProvider.previewLayer.videoGravity = AVLayerVideoGravityResizeAspectFill  ///AVLayerVideoGravityResizeAspect
            self.previewLayerProvider = layerProvider
            self.previewLayerProvider?.previewLayer.addSublayer(rectangeLayer)
...
//            self.captureSession.sessionPreset = AVCaptureSessionPresetPhoto
    ...
    func startRunning(_ errorHandler: ErrorCompletionHandler? = nil) {
        sessionQueue.async {
            if (!self.didSetUp) {
     ...
     */
    func startRecordingMovie(toOutputFileURL url: URL, recordingDelegate: AVCaptureFileOutputRecordingDelegate) throws {
    ...
    /**
     ...
     - Throws: `CaptureManagerError.missingMovieOutput` if `movieFileOutput` is nil.
     */
    func stopRecordingMovie() throws {
        guard let movieFileOutput = movieFileOutput else {
            throw CaptureManagerError.missingMovieOutput
        }
     ...
     */
    func toggleCamera(_ errorHandler: @escaping ErrorCompletionHandler) {
     ...
     */
    func setFlash(_ mode: AVCaptureFlashMode, errorHandler: ErrorCompletionHandler? = nil) throws {
     ...
     */
    func toggleFlash(errorHandler: ErrorCompletionHandler? = nil) throws {
     ...
     */
    func setTorch(_ mode: AVCaptureTorchMode, errorHandler: ErrorCompletionHandler? = nil) throws {
        guard let videoDevice = videoDevice else {
            throw CaptureManagerError.missingVideoDevice
        }
     ...
     */
    func toggleTorch(errorHandler: ErrorCompletionHandler? = nil) throws {
     ...
     */
    func focusAndExposure(at pointInView: CGPoint, errorHandler: ErrorCompletionHandler? = nil) throws {
...
//        dataOutputDelegate?.captureManagerDidOutput(sampleBuffer)
        ...
        DispatchQueue.main.async { [unowned self] in
            self.dataOutputDelegate?.captureManagerDidOutput(sampleBuffer, rectangleLayer: self.rectangeLayer, previewLayerProvider: self.previewLayerProvider)
        }
    }
}
