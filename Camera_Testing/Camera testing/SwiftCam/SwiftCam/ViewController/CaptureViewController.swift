...
final class CaptureViewController: UIViewController, VideoPreviewLayerProvider {
     ...
     */
    var inputs = [CaptureSessionInput.video] {
        didSet {
     ...
     */
    var outputs = [CaptureSessionOutput.stillImage] {
        didSet {
            didChangeInputsOrOutputs()
        }
    }
    ...
    fileprivate lazy var closeButton: UIButton = {
        ...
        let close = UIImage(named: "close", in: bundle, compatibleWith: nil)?.withRenderingMode(.alwaysTemplate)
    ...
    fileprivate lazy var cameraSwitchButton: UIButton = {
        ...
        let bundle = Bundle(for: CaptureViewController.self)
        let switchCamera = UIImage(named: "switchCamera", in: bundle, compatibleWith: nil)?.withRenderingMode(.alwaysTemplate)
    ...
    fileprivate lazy var flashButton: UIButton = {
        ...
        let bundle = Bundle(for: CaptureViewController.self)
        let flashOff = UIImage(named: "flashOff", in: bundle, compatibleWith: nil)?.withRenderingMode(.alwaysTemplate)
    ...
    var detectorViewBorderColor: UIColor = .green {
        didSet {
    ...
    convenience init(inputs: [CaptureSessionInput], outputs:[CaptureSessionOutput]) {
        self.init(nibName: nil, bundle: nil)
        self.inputs = inputs
        self.outputs = outputs
    }
    
    convenience init(inputs: [CaptureSessionInput]) {
        self.init(nibName: nil, bundle: nil)
        self.inputs = inputs
    }
    ...
    fileprivate func setUp() {
        ...
        setUpGestures()
        setUpCaptureManager()
        captureManager.dataOutputDelegate = rectangleDetector
    }
    ...
    fileprivate func setUpCaptureManager() {
        captureManager.setUp(sessionPreset: AVCaptureSessionPresetHigh, previewLayerProvider: self, inputs: inputs, outputs: outputs)
        { (error) in
    ...
    func reactToBarcode(_ mode: BarcodeMode) {
        switch mode {
     ...
     */
    func showIndicatorView(at loc: CGPoint) {
...
// MARK: - animateShadowRadius

private extension CALayer {
    
    func animateShadowRadius(to radius: CGFloat) {
        let key = "com.sic.swiftcam.animateShadowRadius"
